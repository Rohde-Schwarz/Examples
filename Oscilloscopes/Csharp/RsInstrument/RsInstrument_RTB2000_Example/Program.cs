    ...
    class Program
    {
        static void Main()
        {
            ...
            try // try block to catch any RsInstrumentException()
            {
                ...
                // Basic Settings:
                //---------------------------- -------------------------------
                instr.Write("TIM:ACQT 0.01"); // 10ms Acquisition time
                instr.Write("CHAN1:RANG 5.0"); // Horizontal range 5V (0.5V/div)
                instr.Write("CHAN1:OFFS 0.0"); // Offset 0
                instr.Write("CHAN1:COUP ACL"); // Coupling AC 1MOhm
                instr.Write("CHAN1:STAT ON"); // Switch Channel 1 ON
                //-----------------------------------------------------------
                // Trigger Settings:
                //-----------------------------------------------------------
                instr.Write("TRIG:A:MODE AUTO"); // Trigger Auto mode in case of no signal is applied
                instr.Write("TRIG:A:TYPE EDGE;:TRIG:A:EDGE:SLOP POS"); // Trigger type Edge Positive
                instr.Write("TRIG:A:SOUR CH1"); // Trigger source CH1
                instr.Write("TRIG:A:LEV1 0.05"); // Trigger level 0.05V
                ...
                instr.Write("MMEM:CDIR '/INT/'"); // Change the directory
                ...
                instr.Write("MMEM:DEL 'Dev_Screenshot.png'"); // Delete the file if it already exists, otherwise you get 'Execution error'
                instr.QueryOpc();
                instr.ClearStatus();
                instr.InstrumentStatusChecking = true;

                instr.Write("HCOP:LANG PNG;:MMEM:NAME 'Dev_Screenshot'"); // Hardcopy settings for taking a screenshot - notice no file extention here
